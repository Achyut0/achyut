<?php  
  class Expense extends Base {
    ...
    // Smart Budget Splitter: Splits user's entered budget into categories based on past spending
public function splitBudget($UserId, $budget, $days = 30) {
    $stmt = $this->pdo->prepare("
        SELECT Category, SUM(Cost) AS TotalCost 
        FROM expense 
        WHERE UserId = :UserId AND Date >= CURDATE() - INTERVAL :days DAY 
        GROUP BY Category
    ");
    $stmt->bindParam(":UserId", $UserId, PDO::PARAM_INT);
    ...
    foreach ($categoryData as $row) {
        if ($total > 0) {
            $percent = $row->TotalCost / $total;
            $budgetSplit[$row->Category] = round($budget * $percent, 2);
        }
    }
